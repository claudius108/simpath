...
window.$x2 = {
	"version" : "1.0",
	"sequence" : function sequence(args) {
		if ( this instanceof sequence ) {
			this._items = args;
		} else {
			var items = [];
			for (i = 0, il = arguments.length; i < il; i++) {
				items = items.concat(Array.prototype.slice.call(arguments[i], 0));
			}
//			items = items.concat(Array.prototype.slice.call(arguments, 0));
...
$x2.sequence.prototype = {
		"item" : function(nItemIndex) {
			return this._items[nItemIndex - 1];
		},
		"_simpathPath" : function(sXPathExpr) {
			return this._items[0].querySelectorAll(sXPathExpr);
		},
		"_items" : function() {
			return this._items;
		},		
...
var sequenceImpl = function sequenceImpl() {
		this.sequence = "";
	};
	/**
	 * @constructor
	 * @returns {Object}
	 */
	sequenceImpl.prototype.insert = function() {
	};
	sequenceImpl.prototype["delete"] = function() {
	};
	sequenceImpl.prototype.replace = function() {
	};
	sequenceImpl.prototype.setvalue = function() {
	};		
	sequenceImpl.prototype.filter = function() {
	};
	sequenceImpl.prototype.forEach = function() {
// forEach.call(hobbyInputs, function(input){
// console.log(input.querySelector("item").nodeValue);
// });
	};	
	sequenceImpl.prototype.serializeToString = function() {
		return (new XMLSerializer()).serializeToString(this.sequence);
	};	
	// TODO: generate a sequence from an XML document
	// generate a sequence from a sequence literal, containing also other
	// sequences
	

	
/*
 * Definition of XPath expression implementation
 */
...
window.$x = {
	...
	// implementation of XPath Expression object
	"xpathExprImpl" : function(sRefXPathExpr, oXPathContext) {
	...
	"xpath2" : function(sRefXPathExpr, oXPathContext){
		return new this.xpathExprImpl(sRefXPathExpr, oXPathContext);
	},	
				...
				switch(oAjaxOptions.method) {
					...
					case 'post':
						...
						if(xhReq.readyState != 4 || xhReq.status != 200) {
							oSubmissionOptions['simpath-submit-error'](xhReq);
						} else {
							oSubmissionOptions['simpath-submit-done'](xhReq);
						}
					break;
				}
			},
	...
	"_fXPathExprProcessor" : function(sRefXPathExpr, oXPathContext, nCallingFunctionIndex, sValueXPathExpr) {
		...
		// at beginning of XPath expression, which gives the in-scope evaluation
		// context
		...
		for (var i = 0, il = (aFuncMatches != null) ? aFuncMatches.length : 0; i < il; i++) {
			...
			// $("<tr/>", {"html" : "<td>Average time for 1 run for
			// '_fXPathExprProcessor' (ms): " + ( ( new Date()).getTime() -start
			// ) + "</td>"}).appendTo("#results-3 table");
			oXSLTVariables = fXPathFuncHandler[0];
			sRefXPathExpr = fXPathFuncHandler[1];
			// TODO: this is specific to simpath: instance() - have to
			// generalized
	...
	"fn" : {
		"simpath" : {
			"instance" : function(oXSLTVariables, nVariableHash, sFuncMatch, sRefXPathExpr) {
				...
				// $("<tr/>", {"html" : "<td>Average time for 1 run for
				// 'fn.simpath.instance' (ms): " + ( ( new Date()).getTime()
				// -start ) + "</td>"}).appendTo("#results-4 table");
...
$x.compile = function(sRefXPathExpr, oXPathContext, nCallingFunctionIndex, sValueXPathExpr) {
	...
	// $("<tr/>", {"html" : "<td>Average time for 1 run for 'xpath.compile'
	// (ms): " + ( ( new Date()).getTime() -start ) +
	// "</td>"}).appendTo("#results-2 table");
	try {
		...
		// following XPath expression: \"" + sRefXPathExpr + "\"");
	}
...
(function() {
	// function for compiling XPath result consisting of any item but
	// attribute(s) sequence
...
//Notes:
//	"_sequence" : function _sequence(oSequence) {
//		if ( this instanceof _sequence ) {
//			this.sequence = oSequence;
//		} else {
//			return new _sequence(oSequence);
//		}
//	}