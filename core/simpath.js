...
window.$x = {
				...
				switch(oAjaxOptions.method) {
					case 'get':
						...
						if (xhReq.readyState != 4 || xhReq.status != 200) {
							if (oSubmissionOptions['submit-error']) {
								(oSubmissionOptions['submit-error'])(xhReq);
							}							
						} else {
							var oXMLDoc = xhReq.responseXML;
							var ref = oAjaxOptions.ref;
							$x._instances[ref.substring(ref.indexOf("'") + 1, ref.lastIndexOf("'"))] = $x.utils.fCollectNSs(oXMLDoc);
							if (oSubmissionOptions['submit-done']) {
								(oSubmissionOptions['submit-done'])(xhReq);
							}							
						}						
					break;
					case 'post':
	...
	"utils" : {
		...
		"fCollectNSs" : function(oXMLDoc) {
			...
			var utils = $x.utils;
			var sNSs = utils.sNSs;
			var sXMLDoc = sXMLDoc.substring(sXMLDoc.indexOf(" "), sXMLDoc.indexOf(">"));
			var sXMLDoc = sXMLDoc.substring(0,(sXMLDoc.lastIndexOf('/') == sXMLDoc.length - 1) ? sXMLDoc.lastIndexOf('/') : sXMLDoc.length).split(" ");
...
(function() {
	...
	$x._XPathResultFunctions[1] = function(oXPathResult, sAttrName) {
		...
		for ( var i = 0, il = items.length; i < il; i++ ) {
			...
			attribute.value = items[i].getAttribute(sAttrName);
